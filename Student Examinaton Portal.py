...
path = 'D://ProgrammingProject1stSem'
print('-' * 50)
...
def remove(string):
    with open(f'D://PythonProgrammingProject_main-folder/Student.csv', 'r+', newline='') as f:
        script = csv.reader(f)
...
for i in range(student_no):
    ...
    stream = input("Which Stream (e.g. CSE) : ")
    roll = input("Enter the class Roll Number : ")
    ...
    if duplicate('Student.csv', student_id, 0):
        ...
        print(
            'Please enter the subject marks in the above mentioned order in a list type and if you dont have a particular subject write there "null" (e.g. [100,100,"null",75,69,85])')
        ...
        print(f"You can find the report card here : {path}/ReportCards/{student_id}_{''.join(name.split())}.txt")
...
batch = input("Which batch (e.g. 2022-26) : ")
stream = input("Which stream (e.g. CSE) : ")
